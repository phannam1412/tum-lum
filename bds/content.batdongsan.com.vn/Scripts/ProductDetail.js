...
function urlify(c) {
    ...
    try {
        if (c.match(/<\/a>/) != null) {
			...
			if(c.match(/href="https:\/\/diaocnhabe.vn/ig) != null)
			{
				c = c.replace(/href\=\"https\:\/\/diaocnhabe\.vn[^<>]+\"/ig, 'href="#"');
			}
		
            return c;
        }
		
		if(c.toLowerCase().indexOf('diaocnhabe.vn') >= 0)
			return c;
...
$(document).ready(function() {
    ...
    $(".btn-feedback").click(function(a) {
        if ($("#popup_feedback").length == 0) {
            ...
            b += '<div class="fb-input"><span>Họ và tên </span>';
            b += '<input type="text" value="' + userFullName + '" id="popup-name" class="fb-popup"/></div>';
            b += '<div class="fb-input"><span>Email <i>*</i></span>';
            b += '<input type="text" id="popup-email" value="' + userEmail + '" class="fb-popup" /></div>';
            b += '<div class="fb-input"><span>SDT <i>*</i></span>';
            b += '<input type="text" id="popup-mobile" value="' + userMobile + '" class="fb-popup"/></div>';
            b += '<div class="fb-input"><span style="float:left;">Mã bảo vệ <i>*</i></span>';
            ...
            $(".btn-sendfeedback").click(function() {
                ...
                var j = $("#popup-mobile").val().trim();
                if (j.length == 0) {
                    alert("Bạn cần nhập mobile");
                    return
                } else {
                    if (j.length != 10 || !isPhoneNumber(j)) {
                        alert("Mobile nhập vào không đúng");
                        return
                    }
                }
